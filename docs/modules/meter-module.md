...
**Naming convention:** Event names in this table use `meter:` as a documentary module namespace. The wire format uses the Rust `ModuleEvent` enum variant name directly (e.g., `MeasurementReceived`, `RegisterChanged`, `ConfigChanged`), auto-generated by `tauri-specta` into TypeScript union types. The frontend subscribes to typed variants, not string names.

| Spec event name | Wire variant | Payload | Description |
|-------|-------|---------|-------------|
| `meter:detected` | *(planned)* | `Vec<DetectedInstrument>` | Detection completed (auto-triggered on app startup or manual refresh) |
| `meter:connected` | *(planned)* | `ActiveMeterInfo` | Instrument successfully connected |
| `meter:disconnected` | *(planned)* | `{ meter_id: string }` | Instrument disconnected |
| `meter:measurement` | `MeasurementReceived` | `MeasurementResult` | Single measurement completed (from `read` or continuous stream) |
| `meter:health` | *(planned)* | `MeterHealth` | Periodic health/status update |
| `meter:error` | `Error` | `{ meter_id: string, message: string }` | Instrument error |
| `meter:register_changed` | `RegisterChanged` | `RegisterState` | A register slot was updated |
| `meter:config_changed` | `ConfigChanged` | `{ meter_id: string, config: MeterConfig }` | Meter configuration was changed |
| `meter:palette_imported` | *(planned)* | `PaletteSummary` | A palette was successfully imported |
| `meter:chromaticity_update` | *(planned)* | `ChromaticityPoint` | New point for chromaticity diagram (throttled) |
| `meter:tpg_patch_set` | *(planned)* | `{ r: u8, g: u8, b: u8 }` | Confirmation that pattern generator patch was updated |
...
**Event subscription:** The store subscribes to `MeasurementReceived`, `RegisterChanged`, `ConfigChanged`, `ContinuousReadStopped`, `ContinuousReadError`, and `Error` via Tauri `listen()`. *(Planned: detected, connected, disconnected, health, palette_imported, chromaticity_update, tpg_patch_set.)*
...
[User clicks "Read" in StandaloneMeterView]
  → invoke("module_command", { moduleId: "meter", command: "read", payload: { meterId, target } })
    ...
    → [Tauri event] MeasurementReceived
